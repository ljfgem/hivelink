...
import com.linkedin.hivelink.core.utils.FileSystemUtils;
...
public class LegacyHiveTableOperations extends BaseMetastoreTableOperations {
  ...
  protected void doRefresh() {
    try {
      ...
      TableMetadata metadata =
          TableMetadata.newTableMetadataWithoutFreshIds(schema, spec, hiveTable.getSd().getLocation(), tableProperties);
  ...
  Iterable<Iterable<DataFile>> getFilesByFilter(Expression expression) {
    ...
    Iterable<Iterable<DataFile>> filesPerDirectory = Iterables.transform(matchingDirectories, directory -> {
      ...
      return Iterables.transform(files,
  ...
  private List<DirectoryInfo> getDirectoryInfosByFilter(Expression expression) {
    ...
    try {
      ...
      Set<String> partitionColumnNames = current().spec().identitySourceIds().stream()
          .map(id -> current().schema().findColumnName(id)).collect(Collectors.toSet());
      ...
      if (simplified.equals(Expressions.alwaysFalse())) {
        ...
        partitions =
            metaClients.run(client -> client.listPartitionsByFilter(databaseName, tableName, null, (short) -1));
      } else {
        ...
        try {
          ...
          partitions = metaClients
              .run(client -> client.listPartitionsByFilter(databaseName, tableName, partitionFilterString, (short) -1));
              ...
              switch (partitionSchema.fields().get(i).type().typeId()) {
                ...
                case TIMESTAMP:
                  ...
                  record.set(i, LocalDateTime
                      .parse(value,
                          new DateTimeFormatterBuilder().parseLenient().append(DateTimeFormatter.ISO_LOCAL_DATE_TIME)
  ...
  private static DataFile createDataFile(FileStatus fileStatus, PartitionSpec partitionSpec, StructLike partitionData,
    ...
    DataFiles.Builder builder =
        DataFiles.builder(partitionSpec).withPath(fileStatus.getPath().toString()).withFormat(format)
            .withFileSizeInBytes(fileStatus.getLen()).withMetrics(new Metrics(10000L, null, null, null, null, null));
