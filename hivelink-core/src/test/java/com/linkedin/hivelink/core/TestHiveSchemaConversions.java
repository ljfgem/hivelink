...
public class TestHiveSchemaConversions {
  ...
  public void testPrimitiveTypes() {
    List<Type> primitives = Lists.newArrayList(Types.BooleanType.get(), Types.IntegerType.get(), Types.LongType.get(),
        Types.FloatType.get(), Types.DoubleType.get(), Types.DateType.get(), Types.TimestampType.withoutZone(),
        Types.StringType.get(), Types.BinaryType.get(), Types.DecimalType.of(9, 4));

    List<PrimitiveTypeInfo> hivePrimitives = Lists.newArrayList(TypeInfoFactory.booleanTypeInfo,
        TypeInfoFactory.intTypeInfo, TypeInfoFactory.longTypeInfo, TypeInfoFactory.floatTypeInfo,
        TypeInfoFactory.doubleTypeInfo, TypeInfoFactory.dateTypeInfo, TypeInfoFactory.timestampTypeInfo,
        TypeInfoFactory.stringTypeInfo, TypeInfoFactory.binaryTypeInfo, TypeInfoFactory.getDecimalTypeInfo(9, 4));
  ...
  public void testConversions() {
    ...
    check(
        "struct<" + "6: length: optional int, 7: count: optional int, "
            + "8: list: optional list<struct<1: lastword: optional string, 2: lastwordlength: optional int>>, "
            + "9: wordcounts: optional map<string, int>>",
        "struct<" + "length:int,count:int,list:array<struct<lastword:string,lastwordlength:int>>,"
