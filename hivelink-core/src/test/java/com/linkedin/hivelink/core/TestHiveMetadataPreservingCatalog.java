...
public class TestHiveMetadataPreservingCatalog extends HiveMetastoreTest {
  ...
  private Table hiveTable() {
    ...
    storageDescriptor
        .setCols(ImmutableList.of(new FieldSchema("id", "int", ""), new FieldSchema("data", "string", "")));
    ...
    Table tbl = new Table(TABLE_NAME, DB_NAME, System.getProperty("user.name"), (int) currentTimeMillis / 1000,
        (int) currentTimeMillis / 1000, Integer.MAX_VALUE, storageDescriptor, Collections.emptyList(), new HashMap<>(),
        null, null, TableType.EXTERNAL_TABLE.toString());
