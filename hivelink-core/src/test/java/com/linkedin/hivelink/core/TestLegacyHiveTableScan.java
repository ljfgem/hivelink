...
import com.linkedin.hivelink.core.utils.HiveTypeUtil;
...
public class TestLegacyHiveTableScan extends HiveMetastoreTest {
  ...
  private static final List<FieldSchema> DATA_COLUMNS =
      ImmutableList.of(new FieldSchema("strCol", "string", ""), new FieldSchema("intCol", "int", ""));

  private static final List<FieldSchema> PARTITION_COLUMNS =
      ImmutableList.of(new FieldSchema("pcol", "string", ""), new FieldSchema("pIntCol", "int", ""));

  private static final List<FieldSchema> PARTITION_COLUMNS_2 = ImmutableList.of(new FieldSchema("pcol", "string", ""),
      new FieldSchema("pIntCol", "int", ""), new FieldSchema("pDateCol", "date", ""));

  private static final List<FieldSchema> PARTITION_COLUMNS_3 =
      ImmutableList.of(new FieldSchema("pcol", "string", ""), new FieldSchema("pTsCol", "timestamp", ""));

  private static final List<FieldSchema> PARTITION_COLUMNS_4 =
      ImmutableList.of(new FieldSchema("pStringCol", "string", ""), new FieldSchema("pIntCol", "int", ""),
          new FieldSchema("pCharCol", "char(1)", ""), new FieldSchema("pVarcharCol", "varchar(10)", ""),
  ...
  public void testHiveScanMultiPartitionWithFilter() throws Exception {
    ...
    addPartition(table, ImmutableList.of("ds", 1), AVRO, "A");
    addPartition(table, ImmutableList.of("ds", 2), AVRO, "B");
    filesMatch(ImmutableMap.of("pcol=ds/pIntCol=1/A", AVRO, "pcol=ds/pIntCol=2/B", AVRO),
  ...
  public void testHiveScanMultiPartitionWithFilterDate() throws Exception {
    ...
    filesMatch(ImmutableMap.of("pcol=ds/pIntCol=1/pDateCol=2019-04-14/A", AVRO),
  ...
  public void testHiveScanMultiPartitionWithFilterTs() throws Exception {
    ...
    filesMatch(ImmutableMap.of("pcol=foo/pTsCol=2001-09-09T01:46:40.111/A", AVRO), hiveScan(table,
        Expressions.and(Expressions.equal("pCol", "foo"), Expressions.equal("pTsCol", 1000000000111000L))));
  }
  ...
  public void testHiveScanComplexNonStringPartitionQuery() throws Exception {
    ...
    filesMatch(ImmutableMap.of("pStringCol=bar/pIntCol=0/pCharCol=a/pVarcharCol=xy/pDateCol=2020-04-14/F", AVRO),
    ...
    filesMatch(ImmutableMap.of(), hiveScan(table, Expressions.and(Expressions.equal("pdatecol", "2020-04-14"),
  ...
  private static Table createTable(String tableName, List<FieldSchema> columns, List<FieldSchema> partitionColumns,
    ...
    Table tbl = new Table(tableName, DB_NAME, System.getProperty("user.name"), (int) currentTimeMillis / 1000,
        (int) currentTimeMillis / 1000, Integer.MAX_VALUE, storageDescriptor(columns, tableLocation.toString(), format),
        partitionColumns, new HashMap<>(), null, null, TableType.EXTERNAL_TABLE.toString());
    ...
    switch (format) {
      case AVRO:
        ...
        storageDescriptor.setParameters(
            ImmutableMap.of(AvroSerdeUtils.AvroTableProperties.SCHEMA_LITERAL.getPropName(), schemaLiteral(columns)));
        break;
      case ORC:
  ...
  private static Path location(Table table, List<Object> partitionValues) {
    ...
    for (int i = 0; i < table.getPartitionKeysSize(); i++) {
      partitionLocation =
          partitionLocation.resolve(table.getPartitionKeys().get(i).getName() + "=" + partitionValues.get(i));
    }
  ...
  private void addPartition(Table table, List<Object> partitionValues, FileFormat format, String... fileNames)
    ...
    long currentTimeMillis = System.currentTimeMillis();
    metastoreClient.add_partition(new Partition(Lists.transform(partitionValues, Object::toString), table.getDbName(),
        table.getTableName(), (int) currentTimeMillis / 1000, (int) currentTimeMillis / 1000,
        storageDescriptor(table.getSd().getCols(), partitionLocation.toString(), format), new HashMap<>()));
    for (String fileName : fileNames) {
  ...
  private void makePartitionDangling(Table table, List<Object> partitionValues) throws Exception {
    String partitionLocation = metastoreClient
        .getPartition(table.getDbName(), table.getTableName(), Lists.transform(partitionValues, Object::toString))
        .getSd().getLocation();
  ...
  private Map<String, FileFormat> hiveScan(Table table, Expression filter) {
    ...
    CloseableIterable<FileScanTask> fileScanTasks = legacyCatalog
        .loadTable(TableIdentifier.of(table.getDbName(), table.getTableName())).newScan().filter(filter).planFiles();
    return StreamSupport.stream(fileScanTasks.spliterator(), false).collect(Collectors.toMap(f -> {
